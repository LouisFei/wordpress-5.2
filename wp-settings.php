 ...
 * 用于设置和修复公共变量，包括WordPress过程库和类库。
 *
 * Allows for some configuration in wp-config.php (see default-constants.php)
 * 允许在wp-config.php中进行一些配置(参见default-constants.php)
 *
 * @package WordPress
 */
...
require_once( ABSPATH . WPINC . '/plugin.php' ); //插件接口

/*
 * These can't be directly globalized in version.php. 
 * 这些不能在version.php中直接全球化。
 ...
 * 更新时，我们将包含来自另一个安装的version.php，并且不希望这些值在已经设置时被覆盖。
 */
global $wp_version, $wp_db_version, $tinymce_version, $required_php_version, $required_mysql_version, $wp_local_package;
require( ABSPATH . WPINC . '/version.php' );

/**
 * If not already configured, `$blog_id` will default to 1 in a single site configuration. In multisite, it will be overridden by default in ms-settings.php.
 * 如果尚未配置，' $blog_id '在单个站点配置中将默认为1。在multisite中，默认情况下ms-settings.php会覆盖它。
 *
 * @global int $blog_id
 * @since 2.0.0
 */
...
require( ABSPATH . WPINC . '/general-template.php' ); //生成模板
