 ...
 * and filters and hooking functions, and methods. 
 * 插件API位于这个文件中，它允许创建操作、过滤器和挂钩函数以及方法。
 * The functions or methods will then be run when the action or filter is called.
 * 然后在调用操作或过滤器时运行函数或方法。
 *
 * The API callback examples reference functions, but can be methods of classes.
 * API回调示例引用函数，但也可以是类的方法。
 * To hook methods, you'll need to pass an array one of two ways.
 * 要挂接方法，您需要以两种方式之一传递数组。
 *
 ...
 * 通过调用这个函数来调用附加到filter hook $tag的回调函数。
 * 
 ...
 * 通过使用$tag参数指定的新钩子的名称调用这个函数，可以使用这个函数创建一个新的过滤器钩子。
 *
